...
Run a flat binary or a CP/M `.com` program through the CLI:
...
**`src/Z80Emulator.Cli`** is a thin two-mode runner over the core, both modes implemented
as local functions in `Program.cs`:
...
- `cpm` mode loads a CP/M `.com` image at `0x0100` and services BDOS `CALL 5` by trapping
  ...
  console-I/O functions are implemented (1, 2, 6, 9, 10, 11); everything else, notably
  disk/file I/O, is an unimplemented no-op. This exists specifically to host classic
...
**`tests/Z80Emulator.Tests`** is a faster, narrower xUnit suite (ALU flags, branching,
indexed addressing, CB/ED-prefixed instructions, block instructions, interrupts) — useful
for quick iteration, but ZEXDOC/ZEXALL is the higher-confidence correctness check since it
covers documented *and* undocumented flag behavior exhaustively.
